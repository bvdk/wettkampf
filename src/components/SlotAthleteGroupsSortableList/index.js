...
const SortMutation = loader("../../graphql/mutations/updateAthleteGroupSortId.graphql");
...
const SortableItem = SortableElement(({item, index}) => <List.Item>
    <List.Item.Meta
        title={item.name}
        avatar={<div style={{fontSize: "14px"}}>{index+1}</div>}
    />
    <span>
        { item.active ? <Badge style={{marginRight: 10}} status="processing" text={'Aktiv'}/> : undefined}
        <Icon type="drag" />
    </span>
</List.Item>);
...
const SortableList = SortableContainer(({items, loading}) => {
    return (
        <List
            loading={loading}
            dataSource={items}
            renderItem={(item, index) => <SortableItem key={`item-${index}`} index={index} item={item} />}>
        </List>
    );
});
...
class SlotAthleteGroupSortableList extends Component<Props, State> {
  ...
  render() {
    ...
    return <SortableList loading={query.loading || this.state.loading} items={athleteGroups} onSortEnd={this._onSortEnd} />;
  }
}
