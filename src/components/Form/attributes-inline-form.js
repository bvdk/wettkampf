...
import attributesToFormValues from "./attributeValues-to-fomValues";
...
class AttributesInlineForm extends Component<Props, State> {
  ...
  renderAttribute = (attribute, index) => {
    ...
    const { t, attributes, form, inline, customFormItemLayouts, readOnly, mutation, translateMutationOptions} = this.props;
    const {loadingAttributes, errorAttributes, successAttributes} = this.state;
    ...
    if (inline){
      ...
      const values = attributesToFormValues(attributes);
  ...
  renderGroups = (attributes) => {
    ...
    if (groups && groups.length){
      ...
      if (useTabs){
        return <Tabs activeKey={this.state.activeTab || _.get(_.first(groups),'categoryIndex')} onChange={(activeTab)=>this.setState({activeTab})}>
        ...
        return <InnerPadding>
          <Row gutter={16}>
            {groups.map((group: FormGroup, index)=>{
              return <Col className={`form-group-${group.categoryIndex}`} key={`${index}`} {...group.colProps} >
                {group.name ? <GroupTitle className={'AttributesInlineForm-group-title'}>{group.name}</GroupTitle> : null}
  ...
  render() {
    ...
    return <div className={this.props.className} style={{position: 'relative'}}>
      <Form layout={layout} onSubmit={this.handleSubmit}>
        ...
        { !inline && useSubmit ? <FormItem style={{marginTop: 16, ...this.props.submitButtonWrapperStyle}} {...this.offsetFormItemLayout}>
          {this.props.collapse ? <a onClick={this._toggleCollapse}>{t(this.state.collapsed ? 'Display more' : 'Display less')}</a> : null}
          <Button className={this.props.collapse ? "pull-right" : null} type="primary" htmlType="submit" loading={submitting}>
            {this.props.submitTitle ? this.props.submitTitle :'Speichern'}
          </Button>
        </FormItem> : this.props.collapse ? <a style={{display: 'inline-block', padding: Sizes.grid}} onClick={this._toggleCollapse}>{t(this.state.collapsed ? 'Display more' : 'Display less')}</a> : null }
