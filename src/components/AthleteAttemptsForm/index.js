...
const CreateAttemptMutation = loader("../../graphql/mutations/createAttempt.graphql");
const AutoCalcAthletePointsMutation = loader("../../graphql/mutations/autoCalcAthletePoints.graphql");
...
class AthleteAttemptsForm extends Component<Props, State> {
  componentDidMount() {}

  render() {
    const { athleteQuery, disciplines, updateAttemptMutation, createAttemptMutation, autoCalcAthletePointsMutation } = this.props;
    ...
    disciplines.forEach((discipline)=>{
      ...
      for (let i = 1; i<=attemptCount; i++){
        ...
        const attempt = _.chain(athlete).get(`attempts`).filter({discipline}).get(`[${i-1}]`).value();
        attributes.push({
    ...
    return <AttributesInlineForm
      ...
      mutation={(options) => {
        ...
        const promises = Object.keys(updateAttributes).map((key)=>{
          ...
          if (updateAttempt.id){
            return updateAttemptMutation({ variables: { attemptId: updateAttempt.id, data: input }})
          }else {
            return createAttemptMutation({
              ...
              variables: { athleteId: athlete.id, data: input, discipline: updateAttempt.discipline }})
          }
        })

        return Promise.all(promises).then(()=>autoCalcAthletePointsMutation({variables: {id: athlete.id }}));
      }
      }
    />
  }
...
export default compose(
  ...
  withNamespaces(),
  withProps((props)=>({
    disciplines: _.get(props,'eventDisciplinesQuery.event.availableDisciplines') || [_.get(props,'discipline')]
  }))
)(AthleteAttemptsForm);
