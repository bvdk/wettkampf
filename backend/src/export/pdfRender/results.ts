  ...
  return [
    ...
    },
    {
      ...
      render: (text, record) => {
        ...
        return `${birthday.isValid() ? birthday.format("Y") : "-"}/${
...
const getEventResultsDoc = eventId => {
  ...
  const tables = _.chain(Object.keys(groups))
    ...
    .sortBy(key => {
      ...
      if (ageClassId) {
        return _.get(ageClassResolver.ageClass({ id: ageClassId }), "sortId");
      }
      return 0;
    })
    .map(key => {
      ...
      const weightClassTables = _.chain(Object.keys(weightClassGroups))
        .map(weightClassId =>
          weightClassesResolver.weightClass({ id: weightClassId })
        )
        .orderBy(["min"], ["asc"])
