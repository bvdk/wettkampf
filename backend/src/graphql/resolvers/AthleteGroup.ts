...
export default class AthleteGroupResolver
  implements ResolverInterface<AthleteGroup> {
  @FieldResolver()
  public name(@Root() athleteGroup: AthleteGroup) {
    ...
    if (athleteGroup.ageClassId) {
      ...
      const ageClass = ageClassResolver.ageClass({
        id: athleteGroup.ageClassId
      });
      nameComponents.push(ageClass.name);
    }
    if (athleteGroup.weightClassId) {
      const weightClassResolver = new WeightClasses();
      const weightClass = weightClassResolver.weightClass({
        id: athleteGroup.weightClassId
      });
  ...
  @FieldResolver()
  public athletes(@Root() athleteGroup: AthleteGroup, @Args() sort?: SortArgs) {
    if (athleteGroup.athletes) {
      return athleteGroup.athletes;
    }
    return CrudAdapter.filter(Athlete.collectionKey, {
