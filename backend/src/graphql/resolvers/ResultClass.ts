...
export default class ResultClassResolver
  implements ResolverInterface<ResultClass> {
  @FieldResolver()
  public name(@Root() resultClass: ResultClass) {
    ...
    if (resultClass.ageClassId) {
      nameComponents.push(
        _.get(
          ageClassesResolver.ageClass({ id: resultClass.ageClassId }),
          "name"
        )
      );
    }
    ...
    if (resultClass.weightClassId) {
      nameComponents.push(
        _.get(
          weightClassesResolver.weightClass({ id: resultClass.weightClassId }),
          "name"
        )
      );
    }
