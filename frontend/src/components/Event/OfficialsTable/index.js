...
const DeleteOfficialMutation = loader(
  '../../../graphql/mutations/deleteOfficial.graphql'
);
const EventOfficialsQuery = loader(
  '../../../graphql/queries/eventOfficials.graphql'
);

class EventOfficialsTable extends Component<Props, {}> {
  static defaultProps = {
    onEdit: undefined,
    onRemove: undefined,
    onClick: undefined
  ...
  render() {
    ...
    if (onEdit || onRemove) {
      columns.push({
        ...
        render: (text, record) => (
          <span>
            <Link to={`events/${eventId}/officials/${record.id}/edit`}>
              Bearbeiten
            </Link>{' '}
            {this.props.onRemove ? <Divider type="vertical" /> : null}
            {this.props.onRemove ? (
              <span>
                <button
