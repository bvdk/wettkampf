...
import AthleteUpdateForm from '../../../Athlete/UpdateForm';
import AthleteAttemptsTabs from '../../../Athlete/AttemptsTabs';
...
const DeleteAthleteMutation = loader(
  './../../../../graphql/mutations/deleteAthlete.graphql'
);
...
class EventAthleteRoute extends React.Component<Props, State> {
  ...
  _handleDelete = () => {
    ...
    this.setDeleting(true)
      .then(() =>
        this.props.deleteAthleteMutation({
          variables: {
            id: athleteId
          }
        })
    ...
    return (
      <IfRole showError>
          ...
          renderLeft={() => (
            <div>
              ...
              <h3 style={{ display: 'inline', marginLeft: 8 }}>
                {_.get(athleteQuery, 'athlete.name')}
              </h3>
            </div>
          )}
          renderRight={() => (
        ...
        <Tabs
          ...
          onChange={key =>
            history.push(`/events/${eventId}/athletes/${athleteId}/${key}`)
          }>
