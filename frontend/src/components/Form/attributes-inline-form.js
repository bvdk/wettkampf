      ...
      form.validateFields(err => {
        if (!err) {
          ...
          const arr = Object.keys(form.getFieldsValue()).filter(x =>
            form.isFieldTouched(x)
          );
          const values = form.getFieldsValue(fieldKeys || arr);
          const attributeValues = formValueToAttributeValue(
            attributes,
            values,
            false
          );
            ...
            this.setState(
              ...
              () => {
                ...
                if (translateMutationOptions) {
                  promise = mutation(
                    translateMutationOptions(attributeValues, values)
                  );
                } else {
                  promise = mutation({
                    variables: {
      ...
      .filter(
        item =>
          _.get(item, 'categories', [])
            .map(cat =>
              _.isString(cat) ? cat : _.get(cat, 'index', 'default')
            )
    ...
    if (groups && groups.length) {
      if (useTabs) {
        return (
          <Tabs
            activeKey={
              this.state.activeTab || _.get(_.first(groups), 'categoryIndex')
            }
            onChange={activeTab => this.setState({ activeTab })}>
            {groups.map(group => (
              <TabPane tab={group.name} key={group.categoryIndex}>
                {this.filterCategoryAttributes(attributes, group).map(
        ...
        <InnerPadding>
          <Row gutter={16}>
            {groups.map((group: FormGroup, index) => {
              return (
                <Col
                  className={`form-group-${group.categoryIndex}`}
                  key={`${index}`}
                  {...group.colProps}>
                  {group.name ? (
                    <GroupTitle className={'AttributesInlineForm-group-title'}>
                      {group.name}
                    </GroupTitle>
                  ) : null}
  ...
  render() {
    ...
    if (this.props.collapse && this.state.collapsed) {
      attributes = attributes.filter(
        item => this.props.collapse.indexOf(item.index) !== -1
      );
    }
    ...
    return (
      <div
        ...
        style={{
          position: 'relative',
          display: layout === 'inline' ? 'inline' : undefined
        }}>
        <Form layout={layout} onSubmit={this.handleSubmit}>
          {this.renderGroups(attributes)}
          {!inline && useSubmit ? (
            <FormItem
              style={{ marginTop: 16, ...this.props.submitButtonWrapperStyle }}
              {...this.offsetFormItemLayout}>
              {this.props.collapse ? (
                <button className="link-button" onClick={this._toggleCollapse}>
...
export default Form.create({
  onFieldsChange(props, changedFields) {
    if (props.onChange) {
      const reducedChanges = Object.keys(changedFields).reduce((res, cur) => {
      ...
      props.onChange(
        formValueToAttributeValue(props.attributes, completeValues, false)
      );
    }
  },
  mapPropsToFields(props) {
    return FormValueTranslator.translateAttributes(
      props.attributes,
      props.values
    );
  },
