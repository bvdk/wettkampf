...
class FormFactory {
  ...
  static formItemBlackList = ['display', 'section', 'separator'];
  ...
  renderFields() {
    ...
    return Object.keys(this.formConfig.fields)
      .map(key => this.renderField(key, this.formConfig.fields[key]))
      .filter(item => item);
  }
  ...
  renderGroup(group) {
    const fields = Object.keys(this.formConfig.fields)
      .filter(
        key =>
          this.formConfig.fields[key].categories &&
          this.formConfig.fields[key].categories.indexOf(group.key) > -1
      )
      .map(key => ({
  ...
  renderField(name, field, disableFormItem = false) {
    const shouldRenderFormItem =
      !disableFormItem &&
      FormFactory.formItemBlackList.indexOf(field.type) === -1;
    ...
    switch (field.type.toLowerCase()) {
        ...
        result = getFieldDecorator(options.id, {
          ...
          <Input
            ...
            addonAfter={
              ...
              _.isObject(field.config.addonAfter) &&
              field.config.addonAfter.index
                ? this.renderField(
                    field.config.addonAfter.index,
                    {
                      ...field.config.addonAfter,
      ...
      case 'section': {
        result = (
          <div className="pv-10" key={options.key}>
            {field.title ? (
              <label className="bold large-text">{field.title}</label>
            ) : null}
            <hr />
          </div>
        );
          ...
          <CheckboxGroup>
            <Menu>
              {field.options.map((item, index) => (
                <Menu.Item className={'pv-0 ph-5 m-0'} key={index}>
