...
class FormFactory {
  ...
  static renderAttribute(
    ...
    const placeholder = attribute.inputTypeOptions
      ? attribute.inputTypeOptions.placeholder
      : null;
    let addonAfter = attribute.inputTypeOptions
      ? attribute.inputTypeOptions.addonAfter
      : null;
    ...
    switch (attribute.inputType.toLowerCase()) {
      ...
      case 'int': {
        ...
        result = getFieldDecorator(attribute.index, {
          ...
          normalize: value => {
            ...
            if (
              ...
              attribute.type.toLowerCase() === 'int' &&
              /^\d+$/g.test(value)
            ) {
        ...
        })(<Input placeholder={placeholder} addonAfter={addonAfter} />);
        break;
      }
      case 'textArea':
      case 'textarea': {
      ...
      case 'slider': {
        result = getFieldDecorator(attribute.index)(<Slider />);
        break;
      }
      ...
      case 'section': {
        result = (
          <div className="pv-10">
            {attribute.name ? (
              <label className="bold large-text">{attribute.name}</label>
            ) : null}
            <hr />
          </div>
        );
      ...
      case 'daterange': {
        result = getFieldDecorator(attribute.index, {
          rules
        })(<RangePicker disabled={attribute.readonly} format="DD.MM.YYYY" />);
      ...
      case 'dropdowncheckbox': {
        result = getFieldDecorator(attribute.index, {
          ...
          <CheckboxGroup disabled={attribute.readonly}>
            <Dropdown
              {...config}
              overlay={
                config && config.loaderConfig ? (
                  <MenuAsync loaderConfig={config.loaderConfig} />
                ) : (
                  <Menu>
                    {attribute.optionValues.map((item, index) => (
                      <Menu.Item key={item.value}>
                        <Checkbox className={'p-0 m-0'} value={item.value}>
                          {item.name}
                        </Checkbox>
                      </Menu.Item>
                    ))}
              ...
              {_.get(attribute, 'inputTypeOptions.useLabel', false) ? (
                <span>
                  {attribute.name ||
                    _.get(attribute, 'inputTypeOptions.placeholder')}{' '}
                  <Icon type="down" />
                </span>
              ) : (
                <Button>
      ...
      case 'checkbox': {
        if (attribute.optionValues && attribute.optionValues.length) {
          result = getFieldDecorator(attribute.index, {
            ...
            <CheckboxGroup
              options={attribute.optionValues.map(item => ({
                label: item.name,
      ...
      case 'select': {
        result = getFieldDecorator(attribute.index, {
          ...
          config && config.loaderConfig ? (
            <SelectAsync
              ...
              options={
                attribute.optionValues
                  ? attribute.optionValues.map(item => ({
                      label: item.name,
                      value: item.value
                    }))
                  : null
            ...
            <Input disabled={attribute.readonly} placeholder={placeholder} />
          ) : (
            <Select
              disabled={attribute.readonly}
              {...config}
      ...
      case 'autocomplete': {
        result = getFieldDecorator(attribute.index, {
          rules
        })(<Autocomplete {...config} placeholder={placeholder} />);
        break;
      }
    ...
    switch (attribute.inputType.toLowerCase()) {
      ...
      case 'datetimerange': {
        if (value.length === 2) {
          result = (
            <OneLineLabel>
              {moment(value[0]).format('LLL')} -{' '}
              {moment(value[1]).format('LLL')}
            </OneLineLabel>
          );
        } else if (value.length) {
          result = (
            <OneLineLabel>{moment(value[0]).format('LLL')}</OneLineLabel>
          );
        }
      ...
      case 'select': {
        ...
        if (option) {
          ...
          result = (
            <OneLineLabel>
              <LoaderConfigLabel loaderConfig={config.loaderConfig} />
            </OneLineLabel>
          );
