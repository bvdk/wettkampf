...
class SelectSearch extends Component<Props> {
    ...
    render() {
        ...
        if (data.length === 0 && getValueQuery && getValueQuery[valueQueryDataKey]) {
            ...
            data = [
                <Option key={item[valueKey]} name={item[textKey]}
                        value={String(item[valueKey]) || String(item[textKey])} item={item}>
                    {item[textKey]}
                </Option>
            ]
        }
        ...
        if (groupByValue) {
            const groups = _.groupBy(data, (option) => _.get(option.props.item, groupByValue));
            data = Object.keys(groups).map(groupId => <OptGroup key={groupId}
                                                                label={groupByTitle ? _.get(_.first(groups[groupId]), 'props.item.' + groupByTitle) : groupId}
            >
                {groups[groupId]}
            </OptGroup>);
        }
