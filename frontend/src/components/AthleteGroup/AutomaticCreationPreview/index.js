...
import AthleteGroupTable from '../Table';
import Toolbar from '../../Toolbar';
import Strings from '../../../constants/strings';

const AutoCreateAthleteGroupsPreviewQuery = loader(
  '../../../graphql/queries/autoCreateAthleteGroupsPreview.graphql'
);
const AutoCreateAthleteGroupsMutation = loader(
  '../../../graphql/mutations/autoCreateAthleteGroups.graphql'
);
          ...
          tableProps={{
            footer: () => (
              <Toolbar
                renderRight={() => (
                  <Button
                    disabled={!athleteGroups.length}
                    type={'primary'}
                    onClick={this._handleClick}
                    loading={loading}>
                    <b>{`${athleteGroups.length} `}</b>{' '}
                    <span> Startgruppen erstellen</span>
                  </Button>
                )}
              />
            )
...
export default compose(
  ...
  mapProps(props => ({
    ...
    athleteGroups: _.get(
      props,
      'autoCreateAthleteGroupsPreviewQuery.autoCreateAthleteGroupsPreview.AthleteGroups',
      []
    )
  }))
