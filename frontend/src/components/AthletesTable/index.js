...
class AthletesTable extends Component<Props, State> {
  ...
  toggleSelectAll = () => {
    if (
      ...
      this.state.filteredDataSourceIds.length
    ) {
      this.setState(
        {
  ...
  getRowSelection = () => {
    const selectedRowKeys =
      this.props.selectedRowIds || this.state.selectedRowKeys;
  ...
  getColumns(athletes) {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record) =>
          record.resultClass ? (
            record.resultClass.name
          ) : (
            <DangerLabel>Ungültige Wertungsgruppe</DangerLabel>
          ),
      ...
      },
      {
        ...
        render: (text, record) =>
          _.get(record, 'slot') ? (
            _.get(record, 'Slot.name')
          ) : (
            <DangerLabel>Keine Bühne zugeweisen</DangerLabel>
          ),
    ...
    if (this.props.showRowNumber) {
      columns.unshift({
        ...
        render: (text, item, index) =>
          `${_.indexOf(this.state.filteredDataSourceIds, item.id) + 1}`
      });
    }
