...
const waitWhileLoading = (propName = 'data', keys: string[], options) =>
  compose(
    branch(
      props =>
        !_.get(options, 'optional', false) && (!props || !props[propName]),
      renderComponent(() => <ErrorComponent title={'No query'} />)
    ),
    branch(
      props =>
        _.has(props, propName) &&
        (props[propName].loading || props[propName].pending),
      renderComponent(() => (
        <Loader size={_.get(options, 'loaderSize')} loading />
      ))
    ),
    branch(
      props =>
        props[propName] && (props[propName].error || props[propName].rejected),
    ...
    branch(
      props => {
        ...
        return (
          props[propName] &&
          keys &&
          keys.reduce((acc, cur) => acc || !_.has(props[propName], cur), false)
        );
      },
      renderComponent(props => {
        ...
        return (
          <ErrorComponent
            ...
            description={`${keys} could not be found`}
          />
        );
      })
    )
