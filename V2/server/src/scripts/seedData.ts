...
// Declare user and admin role permissions
...
async function createAuth0Accounts(rawData) {
  ...
  // Upgrade first user account to a admin role
  auth0.updateUser(
    { id: accounts[0].user_id },
    {
      app_metadata: {
        groups: [],
        roles: ["admin"],
        permissions: userPermissions.concat(adminPermissions),
      },
    }
  );
...
async function createPosts(rawData, profiles) {
  ...
  for (const profile of profiles) {
    ...
    for (const content of postsContent) {
...
async function createReplies(rawData, profiles, posts) {
  ...
  for (const profile of profiles) {
    ...
    for (const content of repliesContent) {
